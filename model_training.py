...
try:
    ...
    # Query the game logs
    query = "SELECT * FROM raw_player_game_logs;"
    cursor.execute(query)
    
    rows = cursor.fetchall()
    
    # Extract column names dynamically and convert to uppercase
    column_names = [desc[0].upper() for desc in cursor.description]
    combined_gamelogs_df = pd.DataFrame(rows, columns=column_names)

    if not combined_gamelogs_df.empty:
        combined_gamelogs_df['GAME_DATE'] = pd.to_datetime(combined_gamelogs_df['GAME_DATE'])
    
    print(f"{len(combined_gamelogs_df)} game logs loaded.")
...
if combined_gamelogs_df is not None:
    ...
    combined_gamelogs_df = combined_gamelogs_df[combined_gamelogs_df['TEAM_ABBREVIATION'].isin(valid_nba_teams)]
    ...
    combined_gamelogs_df['USG_PCT_SEASON_AVG'] = combined_gamelogs_df.groupby(['SEASON_YEAR', 'PLAYER_ID', 'TEAM_ID'])['USG_PCT'].transform(lambda x: x.expanding().mean().shift(1)).round(1)
    ...
    # 5-Game Baselines
    combined_gamelogs_df['PTS_5G_AVG'] = combined_gamelogs_df.groupby(['PLAYER_ID', 'TEAM_ID'])['PTS'].transform(lambda x: x.rolling(window=5, min_periods=1).mean().shift(1)).round(1)
    combined_gamelogs_df['AST_5G_AVG'] = combined_gamelogs_df.groupby(['PLAYER_ID', 'TEAM_ID'])['AST'].transform(lambda x: x.rolling(window=5, min_periods=1).mean().shift(1)).round(1)
    combined_gamelogs_df['REB_5G_AVG'] = combined_gamelogs_df.groupby(['PLAYER_ID', 'TEAM_ID'])['REB'].transform(lambda x: x.rolling(window=5, min_periods=1).mean().shift(1)).round(1)
    combined_gamelogs_df['MIN_5G_AVG'] = combined_gamelogs_df.groupby(['PLAYER_ID', 'TEAM_ID'])['MIN'].transform(lambda x: x.rolling(window=5, min_periods=1).mean().shift(1)).round(1)
    combined_gamelogs_df['USG_5G_AVG'] = combined_gamelogs_df.groupby(['PLAYER_ID', 'TEAM_ID'])['USG_PCT'].transform(lambda x: x.rolling(window=5, min_periods=1).mean().shift(1)).round(1)
    ...
    # Rolling 10 game opponent averages
    ...
    print("Process failed. No data was returned from the database.")
...
if 'IS_HOME' in future_matchups.columns:
    future_matchups['IS_HOME'] = 0.5
if 'DAYS_REST' in future_matchups.columns:
    future_matchups['DAYS_REST'] = 2.0
...
future_matchups['PROJ_PRA'] = (future_matchups['PROJ_PTS'] + future_matchups['PROJ_AST'] + future_matchups['PROJ_REB']).round(1)

# Clean up, display, and save the final projections
final_cols = ['PLAYER_ID', 'PLAYER_NAME', 'TEAM_ABBREVIATION', 'OPPONENT'] + [col for col in future_matchups.columns if 'PROJ' in col]
...
def insert_matchup_projections(final_projections_df):
    if final_projections_df is None or final_projections_df.empty:
        print("No predictions to insert.")
        return
    
    export_df = final_projections_df
    
    # Order the columns exactly as database
    db_cols = [
        'PLAYER_NAME', 
        'TEAM_ABBREVIATION', 
        'OPPONENT', 
    ...
    try:
        ...
        # Clear out yesterday's projections
        print("Truncating old matchup_projections table...")
        cursor.execute("TRUNCATE TABLE matchup_projections")
        
        # The exact mapping to your SQL schema
        insert_query = """
            INSERT INTO matchup_projections (
                player_name, team_abbreviation, opponent, 
        ...
        print(f"{cursor.rowcount} matchup projections inserted into the database.")

    except Exception as e:
        print(f"Database error during projection export: {e}")
    finally:
        if cursor is not None:
            cursor.close()
        if conn is not None and conn.is_connected():
...
insert_matchup_projections(final_projections)

def update_player_aggregates(final_df):
    if final_df is None or final_df.empty:
        print("No data available to aggregate.")
        return

    df_sorted = final_df.sort_values(by=['PLAYER_ID', 'GAME_DATE'], ascending=[True, False]).copy()
                       
    # Aggregation logic to grab the true recent averages
    def build_aggs(group):
        # Isolate just the current season for the season_fp calculation
        current_season = group[group['SEASON_YEAR'] == group['SEASON_YEAR'].max()]
        
        return pd.Series({
            'player_id': group['PLAYER_ID'].iloc[0],
            'player_name': group['PLAYER_NAME'].iloc[0],
            
            'last_3_points': group['PTS'].head(3).mean(),
            'last_5_points': group['PTS'].head(5).mean(),
            'last_10_points': group['PTS'].head(10).mean(),
            'season_avg_points': current_season['PTS'].mean(),
            
            'last_3_rebounds': group['REB'].head(3).mean(),
            'last_5_rebounds': group['REB'].head(5).mean(),
            'last_10_rebounds': group['REB'].head(10).mean(),
            'season_avg_rebounds': current_season['REB'].mean(),
            
            'last_3_assists': group['AST'].head(3).mean(),
            'last_5_assists': group['AST'].head(5).mean(),
            'last_10_assists': group['AST'].head(10).mean(),
            'season_avg_assists': current_season['AST'].mean()
        })

    agg_df = df_sorted.groupby('PLAYER_ID').apply(build_aggs).reset_index()
    agg_df = agg_df.fillna(0).round(1)
    
    # Database schema
    db_cols = [
        'player_id', 'player_name',
        'last_3_points', 'last_5_points', 'last_10_points', 'season_avg_points',
        'last_3_rebounds', 'last_5_rebounds', 'last_10_rebounds', 'season_avg_rebounds',
        'last_3_assists', 'last_5_assists', 'last_10_assists', 'season_avg_assists'
    ]
    
    export_df = agg_df[db_cols]
    data_to_insert = [tuple(row) for row in export_df.to_numpy()]
    ...
    try:
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute("TRUNCATE TABLE player_aggregates")
        ...
        print(f"{cursor.rowcount} player aggregates inserted.")

    except Exception as e:
        print(f"Database error during aggregates export: {e}")
    finally:
        if cursor is not None:
            cursor.close()
        if conn is not None and conn.is_connected():
