...
if conn is not None:
    ...
    df["last_10_points"] = df.groupby("player_id")["pts"] \
        .transform(lambda x: x.shift(1).rolling(10).mean())

    df["last_5_rebounds"] = df.groupby("player_id")["reb"] \
        .transform(lambda x: x.shift(1).rolling(5).mean())

    df["last_10_rebounds"] = df.groupby("player_id")["reb"] \
        .transform(lambda x: x.shift(1).rolling(10).mean())

    df["last_5_assists"] = df.groupby("player_id")["ast"] \
        .transform(lambda x: x.shift(1).rolling(5).mean())

    df["last_10_assists"] = df.groupby("player_id")["ast"] \
        .transform(lambda x: x.shift(1).rolling(10).mean())
...
(player_id, player_name, slate_date, last_5_fp, season_fp, last_5_minutes, last_10_minutes,
 last_5_points, last_10_points, last_5_rebounds, last_10_rebounds, last_5_assists, last_10_assists, updated_at)
...
ON DUPLICATE KEY UPDATE
    ...
    last_10_points = VALUES (last_10_points),
    last_5_rebounds = VALUES(last_5_rebounds),
    last_10_rebounds = VALUES(last_10_rebounds),
    last_5_assists = VALUES(last_5_assists),
    last_10_assists = VALUES (last_10_assists),
